...
func TestArrayList_Add(t *testing.T) {
	testCases := []struct {
		...
		{
			name:    "add to index OutOfRange -1",
			list:    NewArrayListOf[int]([]int{1, 2, 3}),
			element: 100,
			index:   -1,
			wantErr: fmt.Errorf("gokit: out of range: length: {%d}, and index: {%d}", 3, -1),
		},
		{
			name:    "add to index OutOfRange 100",
			list:    NewArrayListOf[int]([]int{1, 2, 3}),
			element: 100,
			index:   4,
			wantErr: fmt.Errorf("gokit: out of range: length: {%d}, and index: {%d}", 3, 4),
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			err := tc.list.Add(tc.index, tc.element)
...
func TestArrayList_Append(t *testing.T) {
	testCases := []struct {
		...
		{
			name:      "append non-empty list to non-empty list",
			list:      NewArrayListOf[int]([]int{123}),
			elements:  []int{234, 456},
			wantSlice: []int{123, 234, 456},
		},
		{
			name:      "append empty list to non-empty list",
			list:      NewArrayListOf[int]([]int{123}),
			elements:  []int{},
			wantSlice: []int{123},
		},
		...
		{
			name:      "append non-empty list to empty list",
			list:      NewArrayListOf[int]([]int{}),
			elements:  []int{234, 456},
			wantSlice: []int{234, 456},
		},
		{
			name:      "append empty list to empty list",
			list:      NewArrayListOf[int]([]int{}),
			elements:  []int{},
			wantSlice: []int{},
		},
		...
		{
			name:      "append non-empty list to nil list",
			list:      NewArrayListOf[int](nil),
			elements:  []int{234, 456},
			wantSlice: []int{234, 456},
		},
		{
			name:      "append empty list to nil list",
			list:      NewArrayListOf[int](nil),
			elements:  []int{},
			wantSlice: []int{},
		},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			err := tc.list.Append(tc.elements...)
			if err != nil {
				return
			}
...
func TestArrayList_Delete(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			list: &ArrayList[int]{
				elements: []int{123, 100},
			},
			index:   12,
			wantErr: err.IndexOutOfRange(2, 12),
		},
	}
...
func TestArrayList_Get(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			index:   2,
			wantVal: 0,
			wantErr: fmt.Errorf("gokit: 下标超出范围，长度 %d, 下标 %d", 2, 2),
		},
		{
			name:    "index -1",
			list:    NewArrayListOf[int]([]int{123, 100}),
			index:   -1,
			wantVal: 0,
			wantErr: fmt.Errorf("gokit: 下标超出范围，长度 %d, 下标 %d", 2, -1),
		},
	}
...
func TestArrayList_Set(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			element:   5,
			wantSlice: []int{},
			wantErr:   fmt.Errorf("gokit: 下标超出范围，长度 %d, 下标 %d", 5, -1),
		},
		{
			name:      "index  100",
			list:      NewArrayListOf[int]([]int{0, 1, 2, 3, 4}),
			...
			element:   5,
			wantSlice: []int{},
			wantErr:   fmt.Errorf("gokit: 下标超出范围，长度 %d, 下标 %d", 5, 100),
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			err := tc.list.Set(tc.index, tc.element)
			if err != nil {
				assert.Equal(t, tc.wantErr, err)
				return
			}
