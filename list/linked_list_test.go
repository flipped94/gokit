...
func TestLinkedList_Add(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.wantLinkedList.ToSlice(), tc.list.ToSlice())
		})
	}
}
...
func TestLinkedList_Delete(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			list:    NewLinkedListOf[int]([]int{1, 2, 3}),
			index:   99,
			wantErr: fmt.Errorf("gokit: out of range: length: {%d}, and index: {%d}", 3, 99),
		},
		{
			name:    "delete beyond length index 3",
			list:    NewLinkedListOf[int]([]int{1, 2, 3}),
			index:   3,
			wantErr: fmt.Errorf("gokit: out of range: length: {%d}, and index: {%d}", 3, 3),
		},
		{
			name:    "delete empty node",
			list:    NewLinkedListOf[int]([]int{}),
			index:   3,
			wantErr: fmt.Errorf("gokit: out of range: length: {%d}, and index: {%d}", 0, 3),
		},
		{
			name:           "delete index 0",
			list:           NewLinkedListOf[int]([]int{1, 2, 3}),
...
func TestNewLinkedListOf(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		assert.Equal(t, tc.wantedSlice, list.ToSlice())
	}
}
...
func TestLinkedList_Get(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			list:    NewLinkedListOf([]int{1, 2, 3, 4, 5}),
			index:   -1,
			wantErr: fmt.Errorf("gokit: out of range: length: {%d}, and index: {%d}", 5, -1),
		},
		{
			name:    "over right",
			list:    NewLinkedListOf([]int{1, 2, 3, 4, 5}),
			index:   5,
			wantErr: fmt.Errorf("gokit: out of range: length: {%d}, and index: {%d}", 5, 5),
		},
		{
			name:    "empty list",
			list:    NewLinkedListOf([]int{}),
			index:   0,
			wantErr: fmt.Errorf("gokit: out of range: length: {%d}, and index: {%d}", 0, 0),
		},
	}
...
func TestLinkedList_Set(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			index:   7,
			setVal:  888,
			wantErr: fmt.Errorf("gokit: out of range: length: {%d}, and index: {%d}", 7, 7),
		},
		{
			name:    "len(*node) == 0",
			list:    NewLinkedListOf[int]([]int{}),
